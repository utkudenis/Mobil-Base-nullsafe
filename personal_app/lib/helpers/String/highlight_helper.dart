...
/* Example  */
/* DynamicTextBlue getHighlightedText(AppState state, String text, bool isName) {
    CommonHelper.capitalizeString(text);
    return DynamicTextBlue(
      maxLines: 1,
      overflow: TextOverflow.ellipsis,
      text: text,
      highlights: [state.search.searchTextValue],
      style: isName ? apmMediumTextStyle(ApmColors.gray90, fontSize: 14.0, height: 22 / 14.0) : apmMediumTextStyle(ApmColors.gray60, fontSize: 14.0, height: 22 / 14.0),
      caseSensitive: false,
    );
  }*/

class DynamicTextBlue extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    //Main code
    List<TextSpan> _spans = List<TextSpan>.empty(growable: true);
    int _start = 0;

    //For "No Case Sensitive" option
    String _lowerCaseText = text.toLowerCase();
    List<String> _lowerCaseHighlights = List<String>.empty(growable: true);
    ...
    while (true) {
      ...
      if (_highlightsMap.isNotEmpty) {
        List<int> _indexes = List<int>.empty(growable: true);
